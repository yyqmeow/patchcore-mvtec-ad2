...
class MVTecAD2Dataset(torch.utils.data.Dataset):
    """
    PyTorch Dataset for MVTec AD2 dataset (supports all classes: can, fabric, fruit_jelly, rice, sheet_metal, vial, wallplugs, walnuts).
    Similar structure to Vial dataset.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Args:
            source: [str]. Path to the MVTec AD2 data folder (e.g., ./mvtec_ad_2).
            classname: [str]. Name of the class (e.g., 'can', 'fabric', 'fruit_jelly', 'rice', 'sheet_metal', 'vial', 'wallplugs', 'walnuts').
    ...
    def get_image_data(self):
        ...
        class_path = os.path.join(self.source, self.classname)

        if self.split == DatasetSplit.TRAIN:
            # Training data: only good samples
            train_path = os.path.join(class_path, "train", "good")
            ...
            val_path = os.path.join(class_path, "validation", "good")
            ...
            test_good_path = os.path.join(class_path, "test_public", "good")
            test_bad_path = os.path.join(class_path, "test_public", "bad")
            ground_truth_path = os.path.join(class_path, "test_public", "ground_truth", "bad")
            ...
            if os.path.exists(test_good_path):
                ...
                maskpaths_per_class[self.classname]["good"] = [None] * len(good_files)
            ...
            if os.path.exists(test_bad_path):
                ...
                imgpaths_per_class[self.classname]["bad"] = [
                ...
                if os.path.exists(ground_truth_path):
                    ...
                    maskpaths_per_class[self.classname]["bad"] = [
                        os.path.join(ground_truth_path, f) for f in mask_files
                    ]
                else:
                    maskpaths_per_class[self.classname]["bad"] = [None] * len(bad_files)
